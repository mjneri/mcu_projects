...
{
//  This code may not be needed since normally we can't get here without going through a device reset.
//  However, in case we are using alt interrupts and the user just branches here, then this will disable it.
//  It costs nothing and possibly prevents a very difficult to debug problem.
    AIVTEnable(false);
